...
//    }
    ...
    public List<List<VehicleIssue>> findVehicleIssueByVehicleIdAndId(String companyId) {
        List<Vehicle> list1 = vehicleService.getAllVehicle(companyId);
        List<String> vehicleIdList = new ArrayList<>();
        List<List<VehicleIssue>> vehicleIssueList = new ArrayList<>();
        ...
        vehicleIdList.forEach(vehIssue->{
                    if(!vehicleIssueRepository.findAllByVehicleId(vehIssue).isEmpty()){
                        vehicleIssueList.add(vehicleIssueRepository.findAllByVehicleId(vehIssue));
                    }
                }
        );
    ...
    public ResponseEntity<List<VehicleIssue>> findByVehicleID(String vehicleId) {
        List<VehicleIssue> vehicleList=vehicleIssueRepository.findAllByVehicleId(vehicleId);
        if(vehicleList==null){
            return  new ResponseEntity<>(HttpStatus.NO_CONTENT);
        }
