...
public class VehicleClassService {
    ...
    public ResponseEntity<VehicleClass> updateVehicleclass(VehicleClass vehicle, String id) {
        Optional<VehicleClass> vehicleControll = vehicleClassRepository.findById(id);
        if (vehicleControll.isPresent()) {
            vehicleClassRepository.save(vehicle);
