...
public class FleetController {
    ...
    public ResponseEntity<Fleet> getOneFleets(@PathVariable("id")String id) {
        return fleetService.getOneFleets(id);
    }
    ...
    public ResponseEntity addNewFleet(@RequestBody Fleet fleet) throws Exception {
        ...
        if (fleet.getId() != null) {
            ...
            if (fleetInDataBase == null) {
                ...
                if (fleetInDataBase.getId().equals(fleet.getId()) || !fleetInDataBase.getCompanyId().equals(fleet.getCompanyId())) {
                    fleetRepository.save(fleet);
                    return new ResponseEntity<>(fleet, HttpStatus.OK);
                } else {
            ...
            if (!fleetRepository.existsByNameAndCompanyId(fleet.getName(), fleet.getCompanyId())) {
                fleetRepository.save(fleet);
                return new ResponseEntity<>(fleet, HttpStatus.OK);
            } else {
                return new ResponseEntity<>("There is another fleet with this name.", HttpStatus.BAD_REQUEST);
