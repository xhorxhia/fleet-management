...
//                return new ResponseEntity<>(HttpStatus.OK);
  ...
  public ResponseEntity<HttpStatus> deleteAllServiceByVehicleId(String vehicleId){
    ...
    public List<VehicleServices> getAllVehicleServices(List<String> id) {
        List<VehicleServices> vehicleServices = new ArrayList<>();
        id.forEach(ids->{
            Optional<VehicleServices> vhs= serviceRepo.findById(ids);
            if(vhs.isPresent()){
                vehicleServices.add(vhs.get());
            }
        });
